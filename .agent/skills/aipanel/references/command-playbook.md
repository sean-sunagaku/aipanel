...
- `--model`: passed to `claude-code`; when omitted, `.aipanel/profile.yml` `defaultModel` is used before falling back to `sonnet`
...
```bash
node dist/bin/aipanel.js followup --session session_xxx "この方針で続けてよい？" --model sonnet --json
make run ARGS='followup --session session_xxx "この方針で続けてよい？" --model sonnet --json'
```
...
```bash
make run ARGS='debug "この不具合の根本原因は？" --cwd ./target-repo --file src/cache.ts --log logs/error.log --model sonnet --json --timeout 60000'
```
...
- `--model` is visible again in the JSON output and persisted `Run` provider responses
